...
OPTIMIZATIONS_DATA_DIR = DATA_DIR / "optimizations"
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="Device name for output (e.g. 3080Ti, OrinAGX-32GB). Results written to data/optimizations/{device}.json",
    )
    parser.add_argument(
        "--model",
        type=str,
    ...
    # Write results to data/optimizations/{device}.json
    OPTIMIZATIONS_DATA_DIR.mkdir(parents=True, exist_ok=True)
    out_path = OPTIMIZATIONS_DATA_DIR / f"{args.device}.json"
    payload = {
        "device": args.device,
        "model": args.model,
        "imgsz": args.imgsz,
