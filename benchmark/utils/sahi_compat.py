...
#
# MIT License
"""SAHI compatibility wrapper for ultralytics TRT detection."""
...
class UltralyticsTRTDetector(DetectionModel):
    """Wraps an ultralytics YOLO TRT engine for use with the SAHI library."""
    ...
    def _create_object_prediction_list_from_original_predictions(
        self: Self,
        shift_amount_list: list[list[int]] | None = [[0, 0]],  # noqa: B006
        full_shape_list: list[list[int]] | None = None,
    ) -> None:
        shift_amount = (
            shift_amount_list[0]
            if isinstance(shift_amount_list[0], (list, tuple))
            else shift_amount_list
        )
        full_shape = (
            None
