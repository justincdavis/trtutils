...
class TestRawExec:
    ...
    def test_raw_exec_returns_pointers(self, engine, random_input):
    ...
    def test_raw_exec_pointers_are_valid(self, engine, random_input):
        """Returned pointers are non-zero integers (valid GPU allocations)."""
        ptrs = [b.allocation for b in engine.input_bindings]
        result = engine.raw_exec(ptrs, no_warn=True)
        assert all(p != 0 for p in result)

    def test_raw_exec_set_pointers_true(self, engine, random_input):
    ...
    def test_raw_exec_set_pointers_false(self, engine, random_input):
    ...
    def test_raw_exec_output_pointers_match_bindings(self, engine, random_input):
