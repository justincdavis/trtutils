...
def _build_dynamic_test_engine(onnx_name: str, max_batch: int = 4) -> Path:
    """
    Build an engine with a dynamic batch profile (1..max_batch) from a static ONNX.

    The exported test models are static, so the batch dimension is made
    symbolic here rather than shipping a second ONNX asset.
    """
    onnx_path = DATA_DIR / f"{onnx_name}.onnx"
...
@pytest.fixture
def dynamic_engine_path() -> Path:
    """Path to the simple engine built with a 1..4 dynamic batch profile."""
    return SIMPLE_DYNAMIC_ENGINE_PATH
