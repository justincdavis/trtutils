...
class TestPreprocessorLoads:
    ...
    def test_loads(self, make_preprocessor, ptype):
        preproc = make_preprocessor(ptype)
        assert preproc

    @pytest.mark.parametrize("ptype", ["cpu", "cuda", "trt"])
    def test_loads_with_mean_std(self, make_preprocessor, ptype):
...
class TestPreprocessorDuplicate:
    @pytest.mark.parametrize("ptype", ["cpu", "cuda", "trt"])
    def test_duplicate(self, make_preprocessor, ptype, horse_image):
        preproc = make_preprocessor(ptype)
    ...
    @pytest.mark.parametrize("ptype", ["cpu", "cuda", "trt"])
    def test_duplicate_with_mean_std(self, make_preprocessor, ptype, horse_image):
        preproc = make_preprocessor(ptype, mean=IMAGENET_MEAN, std=IMAGENET_STD)
        result1 = preproc.preprocess([horse_image])[0]
        result2 = preproc.preprocess([horse_image])[0]
        assert np.array_equal(result1, result2)
...
class TestPreprocessorParity:
    def _assess_parity(self, preproc1, tag1, preproc2, tag2, method):
    ...
    def test_parity(self, make_preprocessor, ptype, method):
    ...
    @pytest.mark.parametrize("ptype", ["cuda", "trt"])
    @pytest.mark.parametrize("method", ["linear", "letterbox"])
    def test_parity_with_mean_std(self, make_preprocessor, ptype, method):
...
class TestBatchProcessing:
    @pytest.mark.parametrize("ptype", ["cpu", "cuda", "trt"])
    def test_batch_output_shape(self, make_preprocessor, ptype, test_images):
        preproc = make_preprocessor(ptype)
    ...
    @pytest.mark.parametrize("ptype", ["cuda", "trt"])
    def test_batch_parity_with_single(self, make_preprocessor, ptype, test_images):
        preproc = make_preprocessor(ptype)
        images = test_images[:3] if len(test_images) >= 3 else test_images
    ...
    def test_cuda_dynamic_realloc(self, horse_image):
...
class TestPerformance:
    def _measure(self, images, preproc):
        profs = []
        for _ in range(10):
    ...
    def _run_perf_test(self, gpu_preproc):
