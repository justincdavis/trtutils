...
from trtutils.image._detector import OutputSchema
...
def get_detections_yolov10(
    ...
    *,
    ...
    -------
    list[tuple[tuple[int, int, int, int], float, int]]
        The detections from the YOLO-v10 network.
...
def get_detections_efficient_nms(
    outputs: list[np.ndarray],
    conf_thres: float | None = None,
    nms_iou_thres: float = 0.5,
    *,
...
def _postprocess_rfdetr_core(
    ...
    *,
    ...
    # dets: (batch, num_queries, 5) where 5 is [x1, y1, x2, y2, score]
    ...
    bboxes: np.ndarray = dets[0, :, :4]
    scores: np.ndarray = dets[0, :, 4]
    ...
    # Adjust bounding boxes based on ratios and padding
    adjusted_bboxes = bboxes
    adjusted_bboxes[:, 0] = (adjusted_bboxes[:, 0] - pad_x) / ratio_width  # x1
    adjusted_bboxes[:, 1] = (adjusted_bboxes[:, 1] - pad_y) / ratio_height  # y1
    adjusted_bboxes[:, 2] = (adjusted_bboxes[:, 2] - pad_x) / ratio_width  # x2
...
def get_detections_rfdetr(
    ...
    *,
    ...
    """
    Get the detections from the output of an RF-DETR model.
    ...
    -------
    list[tuple[tuple[int, int, int, int], float, int]]
        The detections from the RF-DETR network.
        Each detection is a bounding box in form x1, y1, x2, y2, a confidence score and a class id.
    ...
    results = _get_detections_rfdetr_core(outputs, conf_thres)
...
@register_jit(nogil=True)
def _get_detections_rfdetr_core(
    outputs: list[np.ndarray],
    conf_thres: float | None = None,
) -> list[tuple[tuple[int, int, int, int], float, int]]:
    # set conf_thres to zero if not provided (include all bboxes)
...
def _postprocess_detr_core(
    ...
    *,
    ...
    # DETR outputs ["labels", "boxes", "scores"]
    # labels: (batch, num_queries) containing class IDs
    # boxes: (batch, num_queries, 4) where 4 is [x1, y1, x2, y2]
    # scores: (batch, num_queries) containing confidence scores
    ratio_width, ratio_height = ratios
    pad_x, pad_y = padding
...
def get_detections_detr(
    ...
    *,
    ...
    """
    Get the detections from the output of a DETR-based model (DEIM, RT-DETR, D-FINE).
    ...
    -------
    list[tuple[tuple[int, int, int, int], float, int]]
        The detections from the DETR network.
        Each detection is a bounding box in form x1, y1, x2, y2, a confidence score and a class id.
    ...
    results = _get_detections_detr_core(outputs, conf_thres)
