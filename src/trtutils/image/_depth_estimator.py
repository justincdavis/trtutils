...
class DepthEstimator(ImageModel, DepthEstimatorInterface):
    ...
    def __init__(
        ...
        *,
        ...
        """
        Create a DepthEstimator object.
        ...
        mean : tuple[float, float, float] | None, optional
            ...
            By default, (0.485, 0.456, 0.406) (ImageNet mean).
            Pass None to disable normalization.
        std : tuple[float, float, float] | None, optional
            The standard deviation values to use for the imagenet normalization.
            By default, (0.229, 0.224, 0.225) (ImageNet std).
            Pass None to disable normalization.
        dla_core : int, optional
            The DLA core to assign DLA layers of the engine to. Default is None.
            If None, any DLA layers will be assigned to DLA core 0.
        device : int, optional
            The CUDA device index to use for this depth estimator. Default is None,
        ...
        # prepend with 'depth_' to avoid conflicts with ImageModel._nvtx_tags
        self._nvtx_tags.update(
            {
                "depth_init": f"depth_estimator::init [{self._tag}]",
                "depth_postprocess": f"depth_estimator::postprocess [{self._tag}]",
                "depth_run": f"depth_estimator::run [{self._tag}]",
                "depth_get_depth_maps": f"depth_estimator::get_depth_maps [{self._tag}]",
                "depth_end2end": f"depth_estimator::end2end [{self._tag}]",
                "depth__end2end": f"depth_estimator::_end2end [{self._tag}]",
                "depth__end2end_graph": f"depth_estimator::_end2end_graph [{self._tag}]",
            }
        )

        if FLAGS.NVTX_ENABLED:
            nvtx.push_range(self._nvtx_tags["depth_init"])
    ...
    def postprocess(
        ...
        *,
        ...
        """
        if FLAGS.NVTX_ENABLED:
            nvtx.push_range(self._nvtx_tags["depth_postprocess"])

        if verbose:
            LOG.debug(f"{self._tag}: postprocess")

        t0 = time.perf_counter()
        data = postprocess_depth(outputs, no_copy=no_copy, verbose=verbose)
    ...
    def run(
        ...
        *,
        ...
        """
        if FLAGS.NVTX_ENABLED:
            nvtx.push_range(self._nvtx_tags["depth_run"])
    ...
    def get_depth_maps(
        ...
        *,
        ...
        -------
        np.ndarray | list[np.ndarray]
            For single image: np.ndarray (depth map of shape (1, H, W)).
            For batch: list[np.ndarray] (depth map per image).

        """
        if FLAGS.NVTX_ENABLED:
            nvtx.push_range(self._nvtx_tags["depth_get_depth_maps"])

        if verbose:
            LOG.debug(f"{self._tag}: get_depth_maps")
    ...
    def end2end(
        ...
        *,
        ...
        get_depth_maps in that order. Makes some memory transfer
        ...
        """
        if FLAGS.NVTX_ENABLED:
            nvtx.push_range(self._nvtx_tags["depth_end2end"])
    ...
    def _end2end(
        ...
        *,
        ...
        if FLAGS.NVTX_ENABLED:
            nvtx.push_range(self._nvtx_tags["depth__end2end"])
        ...
        if not isinstance(self._preprocessor, (CUDAPreprocessor, TRTPreprocessor)):
            ...
            if not _is_postprocessed_outputs(outputs):
                err_msg = "Expected postprocessed depth outputs in end2end."
                if FLAGS.NVTX_ENABLED:
        ...
        result = get_depth_maps(postprocessed, verbose=verbose)
    ...
    def _end2end_graph(
        ...
        *,
        ...
        """
        if FLAGS.NVTX_ENABLED:
            nvtx.push_range(self._nvtx_tags["depth__end2end_graph"])

        # Use shared core graph execution
        raw_outputs, _, _ = self._end2end_graph_core(images, verbose=verbose)

        # CPU postprocessing (DepthEstimator-specific)
        postprocessed = self.postprocess(raw_outputs, no_copy=True, verbose=verbose)
        result = get_depth_maps(postprocessed, verbose=verbose)
