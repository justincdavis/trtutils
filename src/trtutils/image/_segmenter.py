...
if TYPE_CHECKING:
    ...
    from .postprocessors._segmentation import Segmentation
...
class Segmenter(Detector, SegmenterInterface):
    ...
    def __init__(
        ...
        *,
        ...
        nms_iou_thres : float
            ...
            By default, 0.5
        mask_thres : float
            The threshold applied to the sigmoid mask logits to produce the
            binary instance masks. By default, 0.5
    ...
    def get_segmentations(
        ...
        *,
        ...
        """
        Get the instance segmentations from postprocessed outputs.
        ...
        conf_thres : float, optional
            The confidence threshold with which to retrieve segmentations.
        ...
        -------
        list[Segmentation] | list[list[Segmentation]]
            For single image: list[Segmentation] (segmentations for single image).
            For batch: list[list[Segmentation]] (segmentations per image).

        """
        if FLAGS.NVTX_ENABLED:
            nvtx.push_range(self._nvtx_tags["seg_get_segmentations"])

        if verbose:
            LOG.debug(f"{self._tag}: get_segmentations")
        ...
        if is_single:
            ...
            result = get_segmentations(batch_outputs, conf_thres, verbose=verbose)
            if FLAGS.NVTX_ENABLED:
                nvtx.pop_range()  # get_segmentations
            return result[0]

        result_batch = get_segmentations(
    ...
    def end2end(  # ty: ignore[invalid-method-override]
        ...
        *,
        ...
        get_segmentations in that order. Makes some memory transfer
        ...
        -------
        list[Segmentation] | list[list[Segmentation]]
            For single image: list[Segmentation] (segmentations).
            For batch: list[list[Segmentation]] (segmentations per image).
        ...
        """
        if FLAGS.NVTX_ENABLED:
            nvtx.push_range(self._nvtx_tags["seg_end2end"])
    ...
    def _end2end(  # ty: ignore[invalid-method-override]
        ...
        *,
        ...
        if FLAGS.NVTX_ENABLED:
            nvtx.push_range(self._nvtx_tags["seg__end2end"])
        ...
        result = self.get_segmentations(postprocessed, conf_thres, verbose=verbose)
    ...
    def _end2end_graph(  # ty: ignore[invalid-method-override]
        ...
        *,
        ...
        """
        if FLAGS.NVTX_ENABLED:
            nvtx.push_range(self._nvtx_tags["seg__end2end_graph"])
