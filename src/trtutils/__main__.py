...
def _build_axonn(args: SimpleNamespace) -> None:
    """
    Build an energy-optimized TensorRT engine using the AxoNN algorithm.

    Uses the AxoNN optimization algorithm to find optimal GPU/DLA layer
    assignments that minimize execution time while staying under an energy target.
    Requires Jetson device with DLA support and INT8 calibration data.
    ...
    args : SimpleNamespace
        ...
        - dla_core : int or None
            DLA core to use (defaults to 0 if not provided).
        - energy_target : float or None
            Target energy in millijoules per inference.
        - energy_ratio : float
            Ratio of GPU energy to use as target (default 0.8).
        - max_transitions : int
            Maximum GPU<->DLA transitions.
    ...
    ------
    ValueError
        If required AxoNN build parameters are missing.

    """
    from trtutils.research.axonn import AxoNNConfig, build_axonn_engine

    # require calibration data for axonn builds (DLA requires INT8)
    if args.calibration_dir is None:
        err_msg = "Calibration directory is required for AxoNN builds"
        raise ValueError(err_msg)
    if args.input_shape is None:
        err_msg = "Input shape is required for AxoNN builds"
        raise ValueError(err_msg)
    if args.input_dtype is None:
        err_msg = "Input dtype is required for AxoNN builds"
        raise ValueError(err_msg)
    ...
    # Create AxoNN config
    config = AxoNNConfig(
        energy_target_mj=args.energy_target,
        energy_target_ratio=args.energy_ratio,
    ...
    LOG.info(f"  GPU layers: {len(schedule.get_gpu_layers())}")
    LOG.info(f"  DLA layers: {len(schedule.get_dla_layers())}")
    LOG.info(f"  Transitions: {schedule.num_transitions}")
    LOG.info(f"  Expected time: {schedule.total_time_ms:.2f}ms")
    LOG.info(f"  Expected energy: {schedule.total_energy_mj:.2f}mJ")
...
def _main() -> None:
    ...
    # build_axonn parser
    build_axonn_parser = subparsers.add_parser(
        "build_axonn",
        help="Build an energy-optimized hybrid GPU/DLA engine using AxoNN algorithm.",
        parents=[general_parser, dla_parser, build_common_parser, calibration_parser, warmup_parser],
    )
    build_axonn_parser.add_argument(
        "--energy_target",
        type=float,
        default=None,
        help="Target energy consumption in millijoules per inference. If not specified, uses energy_ratio of GPU energy.",
    )
    build_axonn_parser.add_argument(
        "--energy_ratio",
        type=float,
        default=0.8,
        help="Ratio of GPU energy to use as target (0.0-1.0). Default is 0.8 (80%% of GPU energy).",
    )
    build_axonn_parser.add_argument(
        "--max_transitions",
        type=int,
        default=3,
        help="Maximum number of GPU<->DLA transitions allowed. Default is 3.",
    )
    build_axonn_parser.add_argument(
        "--profile_iterations",
        type=int,
        default=1000,
        help="Number of profiling iterations for cost estimation. Default is 1000.",
    )
    build_axonn_parser.set_defaults(func=_build_axonn)
