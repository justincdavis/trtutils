...
void scaleSwapTranspose(
    const float* __restrict__ inputArr,
    float* outputArr,
    const float scale,
    const float offset,
    const int height,
    ...
    const int tx = blockIdx.x * blockDim.x + threadIdx.x;  // Height dimension
    const int ty = blockIdx.y * blockDim.y + threadIdx.y;  // Width dimension
    const int tz = blockIdx.z * blockDim.z + threadIdx.z;  // Channel dimension
    if (tx < height && ty < width && tz < 3) {
        const int inputIdx = (tx * width * 3) + (ty * 3) + tz;
        const float val = inputArr[inputIdx];
